...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    fetched = fetch_official_market_indices(timeout_seconds=args.timeout, sources=sources)
    if args.dry_run:
        print(f"market_indices DRY_RUN rows={len(fetched)} sources={','.join(sorted(set(fetched['index_id'].astype(str))))}")
        return 0
    ...
    print(f"market_indices OK rows={len(merged)} official_rows={official_rows} index_ids={index_ids} output={output_path}")
    return 0
