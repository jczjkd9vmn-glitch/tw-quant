...
def test_take_profit_2_sells_remaining_position(tmp_path) -> None:
    engine = _engine()
    _write_trades(tmp_path, [_trade(shares=10, remaining_shares=5, partial_exit_1_done=True)])
    save_daily_prices(engine, _prices({"20260509": 120}))
...
def test_trailing_stop_triggers_after_drawdown_from_high(tmp_path) -> None:
    engine = _engine()
    _write_trades(tmp_path, [_trade(shares=10, highest_price_since_entry=115)])
    ...
    assert row["trailing_stop_price"] == 108.1
...
def test_ma_exit_when_close_breaks_20_day_average(tmp_path) -> None:
    ...
    row = result.updated_trades.iloc[0]
    assert row["status"] == "CLOSED"
    assert row["exit_reason"] == "MA_EXIT"
